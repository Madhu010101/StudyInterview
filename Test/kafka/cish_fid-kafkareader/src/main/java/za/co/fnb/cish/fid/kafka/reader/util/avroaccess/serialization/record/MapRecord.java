...
import za.co.fnb.cish.fid.kafka.reader.util.avroaccess.serialization.SchemaValidationException;
import za.co.fnb.cish.fid.kafka.reader.util.avroaccess.serialization.record.type.ArrayDataType;
import za.co.fnb.cish.fid.kafka.reader.util.avroaccess.serialization.record.type.MapDataType;
import za.co.fnb.cish.fid.kafka.reader.util.avroaccess.serialization.record.util.DataTypeUtils;
import za.co.fnb.cish.fid.kafka.reader.util.avroaccess.serialization.record.util.IllegalTypeConversionException;
    ...
    private Map<String, Object> checkTypes(final Map<String, Object> values, final RecordSchema schema) {
        for (final RecordField field : schema.getFields()) {
            ...
            if (!DataTypeUtils.isCompatibleDataType(value, field.getDataType())) {
                throw new SchemaValidationException("Field " + field.getFieldName() + " has a value of " + value + ", which cannot be coerced into the appropriate data type of " + field.getDataType());
            }
        }
