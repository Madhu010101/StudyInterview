...
<#list eventOriginList as eventOrigin>{
    ...
    "eventOriginName": "za.stress.raw.opm.firstidmasterentity.1",
    "eventOriginSourceSystemCode": <#if !eventOrigin.eventOriginSourceSystemCode?has_content>null</#if><#if eventOrigin.eventOriginSourceSystemCode?has_content>"${eventOrigin.eventOriginSourceSystemCode!""}"</#if>
    }<#sep>,</#list>],
"correlations":[
...
<#assign  employmentsList = body.party.individual.employments>
    ...
    "annualIncomeGrossAmount": <#if !employment.annualIncomeGrossAmount?has_content>null</#if><#if employment.annualIncomeGrossAmount?has_content>"${employment.annualIncomeGrossAmount!""}"</#if>
    }<#sep>, </#list>],
    "educations": [
...
<#list sourceOfFundsList as sourceOfFund>{
    ...
    "sourceOfFundsAmount": <#if !sourceOfFund.sourceOfFundsAmount?has_content>null</#if><#if sourceOfFund.sourceOfFundsAmount?has_content>${sourceOfFund.sourceOfFundsAmount!""}</#if>
    }<#sep>, </#list>],
